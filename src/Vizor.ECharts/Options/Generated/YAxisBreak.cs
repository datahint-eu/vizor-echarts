...
public partial class YAxisBreak
{
    /// <summary>
    /// <![CDATA[
    /// Since v6.0.0   
    ...
    ///     
    /// Note: yAxis.breaks.start and yAxis.breaks.end are the unique identifiers for each break item.
    /// When calling chart.setOption to modify yAxis.breaks.gap or yAxis.breaks.isExpanded , start and end must be specified.
    ...
    /// Using a percentage ensures that the pixel size of the axis break area remains stable, and does not change when yAxis.min , yAxis.max , or dataZoom are modified.
    ...
    /// Therefore, if set as an absolute value, the pixel size of the axis break area will change when yAxis.min , yAxis.max , or dataZoom are modified.
    ///    
    /// Notice: Within a yAxis.breaks array, gap must be specified either entirely in percentages or entirely in absolute values.
